    ...
    <main id="workspace" className="workspace">
      <aside className="search-panel" aria-labelledby="search-title">
        ...
        <form onSubmit={submit} noValidate>
          ...
          <label htmlFor="resume">Your resume <span className="label-detail">{STATIC_DEMO ? 'Private mode only · DOCX' : 'DOCX · up to 5 MiB'}</span></label>
          <div className={`upload-box ${!liveEnabled ? 'disabled-upload' : ''}`}>
            ...
            <p id="resume-help">{liveEnabled ? 'Sent only when you run live analysis. No browser storage; file selection is cleared after the request.' : STATIC_DEMO ? 'Resume upload is available in private mode.' : 'Uploads are disabled in this public-demo configuration.'}</p>
          </div>
